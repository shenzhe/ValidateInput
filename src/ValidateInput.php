...
class ValidateInput
{
    ...
    public static function vaild($str, $type, $errmsg)
    {
        switch (substr($type, 0, 1)) {
            case 'r':
                return self::required($str, $errmsg);
                break;
            case 'e':  //电子邮件
            ...
            default:   //默认按正则
                if (empty($type)) {
                    throw new ValidateException($errmsg ? $errmsg : '错误的类型');
                }
     ...
     * @desc
     */
    public static function required($str, $errmsg = '')
    {
        $str = trim($str);
        if (empty($str)) {
            $errmsg = $errmsg ? $errmsg : '字段不能为空';
    ...
    public static function string($str, $min = 1, $max = 50, $errmsg = '')
    {
        ...
        if ($min && strlen($ret) < $min) {
            throw new ValidateException('至少' . $min . '个字符');
        }
        if ($max && strlen($ret) > $max) {
            throw new ValidateException('至多' . $max . '个字符');
        }
        return $ret;
    }
